...
* @version 1.1
*/
class Task7Controller implements Controller{
	...
	//HEADER
		...
		echo '<div class="container-fluid blue">
				<div class="container title_head center-block">
					<h1 class><strong>Results of the task 7</strong></h1>
				</div>
			</div>';
        ...
        function extractRecursiv($stop,$elem_name,$res,$poursui){
        	...
        	if($stop == false){
                ...
                try {
		            $dbc = SqliteConnection::getInstance()->getConnection();
		            $query = "select element_type_container_id from 'public.element_type_has_relation' where element_type_component_id = :r";
		            $stmt = $dbc->prepare($query);
		        ...
		        if(!empty($res_parent)){
		        	foreach($res_parent as $result){
		        		...
		        		try {
				            ...
				            $stmt->bindValue(':r',$result['element_type_container_id'],PDO::PARAM_STR);
				            $stmt->execute();
